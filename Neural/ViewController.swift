...
class ViewController:  UIViewController, UINavigationControllerDelegate {
  ...
  var currentImage = 0
    ...
    @IBAction func RetinaButton(_ sender: Any) {
        performSegue(withIdentifier: "RetinaSegue", sender: self)
    }
    // MARK: - IBOutlets
  ...
  @IBAction func detect(_ sender: Any) {
    ...
    if let rowIndex = DetectorPickerRow(rawValue: row) {
      switch rowIndex {
      case .detectFaceNameOnDevice:
        detectFaceName(image: imageView.image)
